...
void restart_player(PLAYER *player, PLATFORM platforms[], int n)   //reinicializa o jogador no centro de uma plataforma e indica para o programa que ele está vivo e que não está pulando
{
    ...
    if(check_position(player->position))    //caso a nova posição do jogador seja invalida(por causa da plataforma estar fora da tela, chama a funçao novamente para a plataforma seguinte(n+1)
        restart_player(player, platforms, n + 1);
    player->died = 0;
}
...
PLAYER fall_player(PLAYER player, PLATFORM platforms[], int n)   //retorna o jogador deslocado uma casa para baixo caso ele não esteja nem cima de uma plataforma nem pulando
{
    ...
    if(on_platforms(temp, platforms, n)==-1 && temp.jump<=0)    //verifica se o jogador não está nem cima de uma plataforma nem pulando
    {
...
int gameloop(PLAYER *player, PLATFORM platforms[], int n)   //laço do jogo
{
    ...
    while(player->lives>0)
    {
        ...
        {
            _start = clock();
            restart_player(player, platforms, 0);
            draw_player(*player);
            getch();
            _end = clock();
            tempo -= ((double) (_end - _start)) / CLOCKS_PER_SEC;
        }
        Sleep(SLPTIME);
...
{
    int i, k=0/*, seed = player.position.x + player.position.y*/;
